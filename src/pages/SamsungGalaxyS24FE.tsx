...
const SamsungGalaxyS24FE = () => {
  const [selectedVariant, setSelectedVariant] = useState('8GB/128GB');
  const { addToCart } = useCart();
  const { toast } = useToast();

  const variants = [
    { name: '8GB/128GB', price: 289999 },
    { name: '8GB/256GB', price: 309999 }
  ];
  ...
  const handleAddToCart = () => {
    ...
    addToCart({
      title: `Samsung Galaxy S24 FE (${selectedVariant})`,
      price: selectedPrice,
      image: images[0],
      category: "Samsung"
    });
    ...
    toast({
      title: "Added to Cart! 🛒",
      description: `Samsung Galaxy S24 FE (${selectedVariant}) has been added to your cart`,
            ...
            <div className="space-y-6">
              <div>
                <h1 className="text-4xl lg:text-5xl font-bold mb-4">
                  <span className="text-shimmer">Samsung Galaxy S24 FE</span>
                </h1>
                <div className="flex items-center gap-2 mb-4">
                  {[...Array(5)].map((_, i) => (
                    <Star key={i} className="w-5 h-5 fill-gold-400 text-gold-400" />
                  ))}
                  <span className="text-gray-400 ml-2">(4.7/5 - 218 reviews)</span>
                </div>
              </div>
...
export default SamsungGalaxyS24FE;
